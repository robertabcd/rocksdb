...
class BackupEngineImpl : public BackupEngine {
 ...
 private:
  ...
  class BackupMeta {
   public:
    BackupMeta(const std::string& meta_filename,
        std::unordered_map<std::string, shared_ptr<FileInfo> >* file_infos,
        Env* env)
    ...
    Status AddFile(shared_ptr<FileInfo> file_info);
    ...
    shared_ptr<FileInfo> GetFile(const std::string& filename) const {
    ...
    const std::vector<shared_ptr<FileInfo> >& GetFiles() {
      return files_;
    }
   ...
   private:
    ...
    std::vector<shared_ptr<FileInfo> > files_;
    std::unordered_map<std::string, shared_ptr<FileInfo> >* file_infos_;
    Env* env_;
  ...
  std::unordered_map<std::string, shared_ptr<FileInfo> > backuped_file_infos_;
...
Status BackupEngineImpl::BackupMeta::AddFile(shared_ptr<FileInfo> file_info) {
...
Status BackupEngineImpl::BackupMeta::LoadFromFile(
  ...
  for (uint32_t i = 0; s.ok() && i < num_files; ++i) {
    ...
    uint64_t size;
    const shared_ptr<FileInfo> file_info = GetFile(filename);
    if (file_info) {
      size = file_info->size;
    } else {
